...
class HomeController extends Controller
{
   ...
   //Email verfiy link in send email
    ...
    {   //dd($request->all());
      ...
      //echo  $dob;
        $client = Admin::select('id','dob')->find($request->client_id); //dd($client);
    ...
    public function editclient(Request $request, $encoded_id = NULL)
	{
        ...
        else
		{
			if(isset($encoded_id) && !empty($encoded_id)) {
                ...
                $id = $this->decodeString($encoded_id);
				...
				if(Admin::where('id', '=', $id)->where('role', '=', '7')->exists()) {
                    ...
                    if( \App\ClientContact::where('client_id', $id)->doesntExist() ){
                        if( $fetchedData->phone != "" ) {
                            $oef1 = new \App\ClientContact;
                            $oef1->admin_id = @Auth::user()->id;
                            $oef1->client_id = $id;
                            $oef1->contact_type = $fetchedData->contact_type;
                            $oef1->country_code = $fetchedData->country_code;
                            $oef1->phone = $fetchedData->phone;
                            $oef1->created_at = date('Y-m-d H:i:s');
                            $oef1->updated_at = date('Y-m-d H:i:s');
                            $oef1->save();
                        }
                        ...
                        if( $fetchedData->att_phone != "" ) {
                            ...
                            $oef1->client_id = $id;
                            $oef1->contact_type = '';
                            $oef1->country_code = $fetchedData->att_country_code;
                            $oef1->phone = $fetchedData->att_phone;
                            $oef1->created_at = date('Y-m-d H:i:s');
                            $oef1->updated_at = date('Y-m-d H:i:s');
                            $oef1->save();
                        }
